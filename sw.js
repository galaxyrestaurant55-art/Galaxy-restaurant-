...
var POLL_MS    = 3000;   // 3s poll — max 3s between order and notification
...
self.addEventListener('message', function(e) {
  ...
  switch(e.data.type) {
    ...
    case 'FIREBASE_ACTIVE':
      ...
      _pollTimer = setInterval(pollOrders, 5000);
      break;
    case 'SHOW_NOTIFICATION':
      // Main page asked SW to show notification immediately
      if (e.data.title) {
  ...
  fetch(url, { cache: 'no-store' }).then(function(r) {
    ...
    var promises = newOrders.map(function(o) {
      ...
      return self.registration.showNotification(title, {
        ...
        renotify:           true,
        requireInteraction: true,
        silent:             false,  // use phone default notification sound
        vibrate:            [500,100,500,100,700,200,500,100,500,100,700],
        icon:               'https://api.dicebear.com/7.x/icons/svg?seed=galaxy&backgroundColor=1a0a0a',
        badge:              'https://api.dicebear.com/7.x/icons/svg?seed=galaxy&backgroundColor=1a0a0a',
        actions: [
